...
#coding=utf-8
import os
os.environ['NLS_LANG'] = 'SIMPLIFIED CHINESE_CHINA.UTF8'   #解决从数据库中读取中文乱码的问题

import cx_Oracle
import time


tableName = raw_input(("请输入表名(不区分大小写):").decode("utf-8").encode("gbk")) #输入要查询的表名
moduleName = raw_input(("请输入模块名称(用于给各个类名的前缀):").decode("utf-8").encode("gbk")) #输入模块名称
filePathName = raw_input(("请输入模块所在的文件夹名(用于cshtml指定JS的地址):").decode("utf-8").encode("gbk")) #输入模块所在的文件夹（在PM路径下，如XMZB）
ZWComment= raw_input(("请输入中文注释:").decode("utf-8").encode("gbk")) #输入中文注释
CreatePerson= raw_input(("请输入你的名字拼音缩写:").decode("utf-8").encode("gbk")) #输入创建人拼写缩写
...
#获取表的主键
perimary=myCursor.execute(" SELECT a.constraint_name,  a.column_name FROM user_cons_columns a, user_constraints b WHERE a.constraint_name = b.constraint_name  AND b.constraint_type = 'P' AND a.table_name =UPPER('"+tableName+"')")
arr_P =perimary.fetchall()
myCursor.close()
...
typeDict = dict((['NUMBER','decimal'],['CLOB','string'],['NVARCHAR2','string'],['VARCHAR2','string'],['DATE','DateTime?'],['CHAR','string']))
valueDict = dict((['NUMBER','0.0M'],['CLOB','string.Empty'],['NVARCHAR2','string.Empty'],['VARCHAR2','string.Empty'],['DATE','null'],['CHAR','string.Empty']))
...
def getColumnProperty(columnName,columnType,propertyComment):
	...
	if propertyComment is not None: #判断获取的注释是否为None值
		propertyCommentValue=propertyComment
	propString  = propString+"        /// <summary>\n"
	propString  = propString+"        /// "+propertyCommentValue+"\n"  #此处由于获取的注释中文乱码，所以没有成功使用propertyComment
	propString  = propString+"        /// </summary>\n"
...
fileHeader = fileHeader.replace("XXXX",ZWComment.decode("gbk").encode("utf-8")) #替换文件名
...
wholeFile = wholeFile.replace("$CREATEPERSON$",CreatePerson) #替换文件名

f = open("d:\\PythonAutoCreateCodeFile\\" + currentFileName,"w")
f.write(wholeFile)
f.close()
print (currentFileName+" 生成成功...").decode("utf-8").encode("gbk")
############################################【1】生成Entity 结束##############################################################
...
xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  <statements>
    ...
    <!--修改一条XXXX实体-->
    <update id="$$UPDATEFUNNAME$$" parameterClass="$$ENTITYNAME$$">
      UPDATE $$TABLENAME$$
      SET
      $$UPDATECOLUMNNAME$$
...
def getColumnNameForResultMaps(columnName,i,arrLen,propertyComment):
	propertyCommentValue='''''' 
	if propertyComment is not None: #判断获取的注释是否为None值
		propertyCommentValue=propertyComment
	if i==arrLen:
		valueString = "          <result property=\""+columnName+"\" column=\""+columnName+"\" />  <!--"+propertyCommentValue+"-->"
	if i==1:
		valueString = "  <result property=\""+columnName+"\" column=\""+columnName+"\" />  <!--"+propertyCommentValue+"-->\n"
	else:
		valueString = "          <result property=\""+columnName+"\" column=\""+columnName+"\" />  <!--"+propertyCommentValue+"-->\n"
...
str_PrimaryKey='''''' #主键
if len(arr_P)==0:
	str_PrimaryKey=arr[0][0] #默认第一个字段为主键
else:
	str_PrimaryKey=arr_P[0][1] #获取主键
...
for arrItem in arr:
	i=i+1
	propertyName = arrItem[0]
	propertyComment=arrItem[2] #属性注释
	str_getColumnNameForResultMaps=str_getColumnNameForResultMaps+getColumnNameForResultMaps(propertyName,i,len(arr),propertyComment) ##获取表中的所有列,放到<resultMaps></resultMaps>里
...
fileHeader = fileHeader.replace("FileNameStr",currentFileName) #替换文件名
fileHeader = fileHeader.replace("$CREATEPERSON$",CreatePerson) #替换文件名
...
namespace EMS.PM.DataAccess.Dao
{
    ...
    public class $$DAONAME$$ : PMBaseDao
    {
        ...
        /// Time：NowTimeStr
        /// </summary>
        /// <param name="itemGUID">XXXX实体对象的主键GUID</param>
    ...
    public class $$CONTROLLERNAME$$ : PMBaseController 
    {
        ...
        public string $$INSERTFUNNAME$$()
        {
			...
			try
            {
				...
				//赋值
				item.$$PRIMARYKEY$$= CommonHelper.GetGuid;//自动生成的GUID唯一标识
...
fileMiddle = '''
    ...
    $$MODULENAME$$LB: function () {
        $("#$$MODULENAME$$DataGrid").datagrid({
            url: "../PM$$MODULENAME$$/$$GETLISTFUNNAME$$",
			rownumbers: true, //显示序号
			nowrap: false, //是否换行，True 就会把数据显示在一行里
			striped: true, //True 奇偶行使用不同背景色
			fitColumns: true, // True 就会自动扩大或缩小列的尺寸以适应表格的宽度并且防止水平滚动。
            onDblClickRow: function (rowIndex, rowData) {//双击是触发的事件，弹出窗口
				$$MODULENAME$$.Show$$MODULENAME$$Info(rowData.$$PRIMARYKEY$$,'VIEW');
            },
            columns: [[
                 '''
fileFooter='''
    ...
    $$MODULENAME$$Save: function () {
        ...
        if (CheckDataValid('#form')) {//验证表单数据
            var postData = { "$$MODULENAME$$": JSON.stringify($$MODULENAME$$.vm$$MODULENAME$$.data.$model) }; //将data数据为Controller中的XXXX实体类赋值
    ...
    //点击【删除】按钮，执行删除
    $$MODULENAME$$Del: function (itemGUID) {
        getDataAsync("../PM$$MODULENAME$$/Delete$$MODULENAME$$?itemGUID=" + itemGUID, "POST", "", function (result) {//调用Controller中的删除方法
            if (result == true) {
    ...
    Get$$MODULENAME$$Info: function () {
        ...
        var itemGUID = getQueryString("itemGUID"); //guid
        var url = "../PM$$MODULENAME$$/Get$$MODULENAME$$ByGUID?itemGUID=" + itemGUID; //根据字典表的ID查询字典表的数据
        if (editFlag == "VIEW") {//查看页面
        ...
        getDataAsync(url, "POST", "", function (data) {
			...
			//可以使用data[0][""]获取具体的值，并赋给界面控件值
        });
    },
	//显示编辑或查看页面
    Show$$MODULENAME$$Info: function (itemGUID,flag) {
			var winTitle='XXXX-编辑';
			if(flag=='View')
				winTitle='XXXX-查看';
            var url = url = "../PM$$MODULENAME$$/$$MODULENAME$$MX?itemGUID=" +  + "&EditFlag="+flag;
...
function del() {
    ...
    if (dataGridItem != null) {
        var itemGUID= dataGridItem.$$PRIMARYKEY$$; //$$PRIMARYKEY$$:GUID唯一标识
...
function edit() {	
    ...
    $$MODULENAME$$.Show$$MODULENAME$$Info(dataGridItem.$$PRIMARYKEY$$,'EDIT');
}
    
'''
#数据类型转换
typeDict = dict((['NUMBER','0'],['CLOB','\"\"'],['NVARCHAR2','\"\"'],['VARCHAR2','\"\"'],['DATE','\"\"'],['CHAR','\"\"']))

#getColumnName方法：获取表中的所有列
def getColumnName(columnName,columnType,i,arrLen,propertyComment):
	propertyCommentValue='''''' 
	if propertyComment is not None: #判断获取的注释是否为None值
		propertyCommentValue=propertyComment
	if i==arrLen:
		valueString = "            \"" + columnName +"\":"+typeDict[columnType]+"   //"+propertyCommentValue+"\n"
	else:
		valueString = "            \"" + columnName +"\":"+typeDict[columnType]+",  //"+propertyCommentValue+"\n"
	return valueString;

#getColumnNameForDataGrid:为DataGrid构造列
def getColumnNameForDataGrid(columnName,i,arrLen,propertyComment):
	propertyCommentValue='''''' 
	if propertyComment is not None: #判断获取的注释是否为None值
		propertyCommentValue=propertyComment
	if i==arrLen:
		valueString = "            {field: \""+columnName+"\", title: \""+propertyCommentValue+"\", width: 30 }  \n"
	else:
		valueString = "            {field:\""+columnName+"\", title: \""+propertyCommentValue+"\", width: 30 }, \n"
	return valueString;
...
for arrItem in arr:
	i=i+1
	propertyName = arrItem[0]
	columnType=arrItem[1]
	propertyComment=arrItem[2] #字段注释
	fileBody = fileBody + getColumnName(propertyName,columnType,i,len(arr),propertyComment)
	fileMiddle = fileMiddle + getColumnNameForDataGrid(propertyName,i,len(arr),propertyComment)
...
wholeFile = wholeFile.replace("$$PRIMARYKEY$$",str_PrimaryKey) #替换主键
wholeFile = wholeFile.replace("$$MODULENAME$$",moduleName) #替换moduleName
wholeFile = wholeFile.replace("NowTimeStr",currentTime) #替换成当前时间
wholeFile = wholeFile.replace("XXXX",ZWComment.decode("gbk").encode("utf-8")) #替换文件名
wholeFile = wholeFile.replace("$CREATEPERSON$",CreatePerson) #替换文件名
...
fileHeader = fileHeader.replace("$$FILEPATHNAME$$",filePathName) #替换文件路径
fileHeader = fileHeader.replace("XXXX",ZWComment.decode("gbk").encode("utf-8")) #替换文件名
fileHeader = fileHeader.replace("$CREATEPERSON$",CreatePerson) #替换作者名
...
//----------------------------------------------------------------*/ \n
	...
	(1)BusinessFactory.cs文件：
		...
		方式：添加
		代码：
			#region XXXX 【EditBy $CREATEPERSON$,NowTimeStr】
	  ...
	  (2)PM.Maps.config文件
			...
			方式:添加(路径按实际情况编写)
			代码：
				<!--XXXX [EditBy $CREATEPERSON$,NowTimeStr]-->
	  ...
	  (3)PMViewEngine.cs文件
			...
			方式：添加(路径按实际情况编写)
			代码：
				 "~/Views/PM/$$FILEPATHNAME$$/{0}.cshtml",   //XXXX [EditBy $CREATEPERSON$,NowTimeStr]
...
f.write(wholeFile)
f.close()
print (moduleName+"使用必读.txt 生成成功...").decode("utf-8").encode("gbk")


############################################【10】补充文件 结束##############################################################


print ("js文件和cshtml文件需要用系统记事本打开，然后另存为utf-8格式，否则会在页面中出现中文乱码的问题").decode("utf-8").encode("gbk")
print ("全部文件生成成功,位于D盘PythonAutoCreateCodeFile文件夹下...按任意键退出").decode("utf-8").encode("gbk")
raw_input()
